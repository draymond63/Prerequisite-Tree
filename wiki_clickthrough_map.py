...
def _get_ct_links(path = 'datasets/raw/clickstream-enwiki-2023-05.tsv') -> pd.DataFrame:
    ...
    df = df[df['type'] == 'link']
    df = df.filter(items=['source', 'target', 'n'])
...
class WikiMap:
    ...
    def get_clickthrough_rates(self, wiki_id: str, target_normalized = False, source_normalized = False) -> pd.Series:
        ...
        if len(ctr) == 0:
            raise ValueError(f'Article not found: {wiki_id}')
        # Normalize based on source clicks (i.e. portion of source traffic going to target)
        # This favours popular links, typically related
        if source_normalized:
            ctr /= ctr.sum()
        # Normalize based on target clicks (i.e. portion of traffic target is coming from source)
...
if __name__ == '__main__':
    m = WikiMap()
    print(m.get_clickthrough_links('control_theory').head(10))
    ctr = m.get_clickthrough_rates('control_theory', target_normalized = True, source_normalized = True)
